...
function ImageToText() {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      setResults((prev) => {
        // Find the last loading card with this filename
  ...
  return (
    <div className="app-home">
      ...
      <Link to="/text-extraction" className="nav-link">Text Extraction</Link>
        <Link to="/image-to-text" className="nav-link nav-active">Images</Link>
          ...
          <div className="image2text-left">
            <form className="file-uploader-card glassmorph-uploader" onSubmit={handleUpload}>
              <div className="uploader-illustration">
                <span role="img" aria-label="upload" style={{fontSize: '3.5rem'}}>🖼️</span>
              </div>
              <h2>Image to Text</h2>
              <input type="file" accept="image/*" className="file-input fancy-input" onChange={handleFileChange} />
              ...
              <p className="file-uploader-desc">Select an image file (JPG, PNG, etc.) to extract text from.</p>
