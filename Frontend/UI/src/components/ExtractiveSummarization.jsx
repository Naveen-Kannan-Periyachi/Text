...
function ExtractiveSummarization() {
  ...
  const handleUpload = async (e) => {
    ...
    formData.append("summary_type", "extractive");
    try {
      ...
      if (res.ok && data.summaries) {
        ...
        await fetch("/api/actions/document", {
          ...
          body: JSON.stringify({ filename: file.name + " (Extractive Summary)", content: summaryText })
        });
      }
    } catch {
      setResults((prev) => {
  ...
  return (
    <div className="app-home">
      <Navbar active="summarization">
        ...
        <Link to="/extractive-summarization" className="nav-link nav-active">Extractive Summarization</Link>
      </Navbar>
        ...
        <section className="main-section image2text-section">
          <div className="image2text-left">
            <form className="file-uploader-card glassmorph-uploader" onSubmit={handleUpload}>
              ...
              <p className="file-uploader-desc">Upload a document to generate an extractive summary.</p>
...
export default ExtractiveSummarization;